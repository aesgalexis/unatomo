...
(function(){
  ...
  for (var i=0; i<Math.min(abList.length,118); i++){
    ...
    var title = ab.title || "Atomic Button";
    var body  = ab.body  || "";
    var isDefault = !!ab.isDefault;
    infos.push({ color: col, title: title, body: body, isDefault: isDefault });
  }
  if (infos.length === 0) {
  ...
  function setNucleusFromInfos(arr){
    ...
    for (var i=0; i<Z; i++){
      ...
      p.userData = { isProton:true, payload:{ title: inf.title, body: inf.body, isDefault: inf.isDefault, ts: abTs } };
