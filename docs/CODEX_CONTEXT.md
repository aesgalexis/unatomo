...
- The same collaborator flag enables `#/todo`; To Do has no separate admin
  ...
  owner-only deletion.
- The `superadmin` control panel has a `Respaldo` / `Backup` card. It reads `static/data/nfc-backup-status.json`, which is updated by the local NFC backup scripts. Prefer `npm.cmd run backup:nfc:all`; the panel shows its aggregate status, age, included scope, and explicitly pending recovery scopes above the Firestore and Storage details.
