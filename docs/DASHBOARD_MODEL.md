...
- While viewing `#/registro`, grouped blocks with activity newer than `registrySeenAt` render all their visible rows with stronger text, so the parent event and its related notes/completion/status lines stay visually together. The badge count uses the same block-level rule and sums the visible rows inside unseen blocks. They are marked seen when the user leaves `#/registro`, not immediately on entry.
