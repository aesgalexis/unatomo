...
- While viewing `#/registro`, rows newer than `registrySeenAt` render with stronger text. Grouped blocks do not mark every child as unseen; the badge count and highlighted rows must match by each event timestamp. They are marked seen when the user leaves `#/registro`, not immediately on entry.
