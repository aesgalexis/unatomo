...
export const EMAIL_TEMPLATE_DEFINITIONS: EmailTemplateDefinition[] = [
  ...
  {id: "password_changed", category: "security", hasButton: false, integration: "active", labels: {es: {name: "Contraseña modificada", description: "Confirma un cambio de contraseña y alerta si no fue reconocido."}, en: {name: "Password changed", description: "Confirms a password change and warns if it was not recognised."}}},
  ...
  {id: "email_change_old", category: "security", hasButton: false, integration: "active", labels: {es: {name: "Aviso al correo anterior", description: "Confirma en la dirección anterior que el cambio se ha completado."}, en: {name: "Previous email notice", description: "Confirms to the previous address that the change is complete."}}},
  {id: "email_change_new", category: "security", hasButton: true, integration: "active", labels: {es: {name: "Verificar correo nuevo", description: "Confirma la nueva dirección antes de completar el cambio."}, en: {name: "Verify new email", description: "Confirms the new address before completing the change."}}},
...
export const renderEmailTemplate = (
  ...
  case "email_change_old": {
    const subject = en ? "Your Unatomo email was changed" : "Tu correo de Unatomo ha cambiado";
    const maskedEmail = clean(input.newEmail, en ? "ne***@example.com" : "nu***@ejemplo.com");
    return build(subject, shell({title: en ? "Email address changed" : "Dirección de correo modificada", preheader: subject, paragraphs: [en ? `Your account now uses ${escapeHtml(maskedEmail)}.` : `Tu cuenta utiliza ahora ${escapeHtml(maskedEmail)}.`, en ? "If you do not recognise this action, contact info@unatomo.com immediately." : "Si no reconoces esta acción, contacta inmediatamente con info@unatomo.com."]}), `${greeting}\n\n${subject}\n${maskedEmail}`);
  }
