...
export const getMachineAccessPublic = onCall(async (request) => {
...
export const verifyMachineAccessUser = onCall(async (request) => {
  const tagId = (request.data?.tagId || "").toString().trim();
  ...
  const tenantId = (access.tenantId || access.ownerUid || "").toString().trim();
...
export const updateMachineAccessOperational = onCall(async (request) => {
  const tagId = (request.data?.tagId || "").toString().trim();
  ...
  if (tagId.length > 80 || sessionId.length > 80 || sessionToken.length > 120) {
  ...
  if (!["operativa", "fuera_de_servicio"].includes(status) || !logs || !tasks) {
